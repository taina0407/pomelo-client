...
(function () {
  ...
  pomelo.init = function (params, cb) {
    ...
    pomelo.socketIO = io.initSocket(url, {'force new connection': false, reconnect: true});

    pomelo.socketIO.on('connect', function () {
    ...
    pomelo.socketIO.on('reconnect', function () {
      console.log('reconnect');
    });

    pomelo.socketIO.on('message', function (data) {
    ...
    pomelo.socketIO.on('error', function (err) {
      console.log(err);
    });

    pomelo.socketIO.on('disconnect', function (reason) {
      console.log('[pomeloclient.init] websocket disconnected!');
      pomelo.emit('disconnect', reason);
    });
  };

  // pomelo.socket = function () {
  //   return socket;
  // };

  pomelo.isConnected = function () {
    return (pomelo.socketIO && pomelo.socketIO.socket && pomelo.socketIO.socket.connected) || false;
  };

  pomelo.isConnecting = function () {
    return (pomelo.socketIO && pomelo.socketIO.socket && pomelo.socketIO.socket.connecting) || false;
  };

  pomelo.isReconnecting = function () {
    return (pomelo.socketIO && pomelo.socketIO.socket && pomelo.socketIO.socket.reconnecting) || false;
  };

  pomelo.isOpen = function () {
    return (pomelo.socketIO && pomelo.socketIO.socket && pomelo.socketIO.socket.open) || false;
  };

  pomelo.disconnect = function () {
    if (pomelo.socketIO) {
      pomelo.socketIO.disconnect();
      pomelo.socketIO = null;
    }
  };
